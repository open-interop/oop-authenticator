...
class DeviceManager extends EventEmitter {
    ...
    async getDevices(retries = 0) {
        if (retries >= MAX_RETRIES) {
            throw new Error("Max retries reached, unable to fetch devices from core.");
        }

        try {
            const res =  await fetch(`${this.config.oopCoreApiUrl}/devices/auth`, {
                headers: { "X-Core-Token": this.config.oopCoreToken }
            });

            if (res.status < 200 || res.status > 299) {
                throw new Error(`Result status "${res.status}" outside 200 range.`);
            }

            const data = await res.json();

            if (!(data instanceof Array)) {
                throw new Error(`Got data type "${typeof data}", which is not iterable.`);
            }
